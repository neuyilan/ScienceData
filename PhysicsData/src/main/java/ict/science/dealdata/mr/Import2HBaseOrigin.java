...
public class Import2HBaseOrigin {

	
	private static String cf="cf";
	private static String token = ","; 
	private static String qualifier[]={"rowkey","entry","runNo","eventId","totalCharged","totalNeutral","totalTrks"};
		...
		public void map(Object key, Text value,Context context) throws IOException, InterruptedException{
			...
			if(temp.length!=7){
				...
				for(int i =1;i<temp.length;i++){
					kv = new KeyValue(Bytes.toBytes(temp[0]),Bytes.toBytes(cf),Bytes.toBytes(qualifier[i]),Bytes.toBytes(temp[i]));
					if(null !=kv){
